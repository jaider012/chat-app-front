...
export class KeyManager {
  ...
  async initializeStorage(): Promise<void> {
    return new Promise((resolve, reject) => {
      const request = indexedDB.open(
        KeyManager.INDEXEDDB_NAME,
        KeyManager.INDEXEDDB_VERSION
      );
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(KeyManager.STORE_NAME)) {
          const store = db.createObjectStore(KeyManager.STORE_NAME, {
  ...
  async deriveSharedSecret(publicKey: CryptoKey): Promise<CryptoKey> {
    ...
    return await WebCryptoUtils.deriveSharedSecret(
      userKeys.privateKey,
      publicKey
    );
  }
  ...
  async storeConversationKey(
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(new Error("Failed to store conversation key"));
  ...
  async removeConversationKey(conversationId: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(new Error("Failed to remove conversation key"));
      request.onsuccess = () => resolve();
    });
  }
